...
kable(head(analiza %>% select(Drzava, Leto, Cepljenost.Z, Cepljenost.M)))
```
...
Največji odstotek cepljenih fantkov v 1. letu starosti je v Jordanu (93.5%), prav tako deklic (92.7%). Najmanjši odstotek cepljenih fantkov (izmed držav, ki jih analiziram), je v Nigeriji (26.0%), prav tako deklic (25.2%). Sledi Demokratična republika Kongo.


```{r graf6, echo=FALSE,fig.align='center',fig.cap='Graf odstotek nerazvitih otrok v državah z največjim številom umrlih'}
ggplot(data=analiza,aes(x=Drzava, y=`Odstotek nerazvitih otrok`)) + geom_point(colour='blue',size=4)
```

Graf prikazuje odstotek nerazvitih otrok do 5. leta starosti v državah z največjim številom umrlih. Razvidno je, da je največji delež (47.8%) nerazvitih otrok v Malaviju (južna Afrika), sledi Demokratična republika Kongo, zelo blizu pa sta tudi Mozambik (prav tako južna Afrika) in Bangladeš, ostale države pa imajo delež nerazvitih otrok manjši od 40%.
...
ggplot(data=analiza,aes(x=Drzava, y=`Odstotek podhranjenih otrok`))+ geom_point(colour='green', size= 4)
```

Iz grafa opazimo, da je največji odstotek podhranjenih otrok do 5. leta starosti v Bangladešu, ta je namreč kar 36.8%. Za njim je Nigerija (31.0%), ostale države pa imajo delež podhranjenih otrok manjši od 30%.
